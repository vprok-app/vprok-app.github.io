...
// The Firebase JS SDK, served from our own origin (ADR-0008) and loading while
// the engine loads. FlutterFire skips its www.gstatic.com injection when
// window.firebase_core is already set; lib/integrations/firebase awaits this
// promise before Firebase.initializeApp. firebase-app.js goes first: the other
// bundles import it, and concurrent imports trip a WebKit module bug
// (flutterfire#18436). Version: kFirebaseJsSdkVersion, checked by a test.
window.vprokFirebaseReady = (async () => {
  const sdk = './firebasejs/12.17.0/';
  window.firebase_core = await import(sdk + 'firebase-app.js');
  const [auth, firestore, appCheck] = await Promise.all([
    import(sdk + 'firebase-auth.js'),
    import(sdk + 'firebase-firestore-pipelines.js'),
...
// With the backend off nobody awaits the promise; mark a failure handled so it
// is not reported as unhandled. lib/integrations/firebase still sees it.
window.vprokFirebaseReady.catch(() => {});

// Start the app without service-worker settings (ADR-0001). Flutter's service
// worker is deprecated and a stub that unregisters itself; the default
// bootstrap still registered it on every launch and waited for its activation
// before starting the app — measured in the F-001 spike.
// CanvasKit from our own build, never from www.gstatic.com (Flutter's default;
// ADR-0008): same reason as the Firebase SDK above, and it is the largest
// download on the cold path.
